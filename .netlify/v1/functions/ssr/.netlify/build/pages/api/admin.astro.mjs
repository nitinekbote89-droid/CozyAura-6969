...
import { s as sendOrderShipped } from '../../chunks/email_BVOhTcpL.mjs';
export { renderers } from '../../renderers.mjs';

const supabase = createClient("https://fxihqzepiayehvszyita.supabase.co", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZ4aWhxemVwaWF5ZWh2c3p5aXRhIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MTcxMjc5MCwiZXhwIjoyMDk3Mjg4NzkwfQ.Rbke2hVSbPZA5dp-XdLrlrFRTwnpRAnxTjke7RjOEPg");
...
async function uploadToCloudinary(base64Payload, identifierToken) {
  try {
    const cloudName = "deein4hop";
    const apiKey = "286614694633171";
    const apiSecret = "-0Khc2pZjuP44CPIx9U6R71C0xY";
...
async function executeCloudinaryCleanup() {
  try {
    ...
    const activePublicIds = /* @__PURE__ */ new Set();
    if (dbProducts) {
...
function runCleanupInBackground() {
  ...
  executeCloudinaryCleanup().then((res) => res.json()).then((json) => {
...
function startAutomaticCleanupSchedule() {
  ...
  console.log(`[Scheduler] Image cleanup scheduled daily at 3:00 PM. Next run in ${(delay / 1e3 / 60 / 60).toFixed(2)} hours.`);
  setTimeout(() => {
    runCleanupInBackground();
    setInterval(runCleanupInBackground, 24 * 60 * 60 * 1e3);
  }, delay);
}
...
async function GET({ request }) {
  try {
    ...
    const adminSecret = "CozyAura@6969";
    if (!adminSecret) ;
    ...
    if (action === "get_customer_profile") {
      ...
      const [{ data: addrs }, { data: orders2 }, { data: wishlist2 }] = await Promise.all([
      ...
      if ((!fname || !lname) && orders2 && orders2.length > 0) {
        const latestOrder = orders2[0];
      ...
      const customer = {
        ...
        wishlistCount: (wishlist2 || []).length,
        recentOrders: (orders2 || []).map((o) => ({
          id: o.id,
          total: o.total,
          status: o.status,
          date: o.date,
    ...
    if (action === "get_gift_card_layout") {
      ...
      const { data: layout, error } = await supabase.from("gift_card_layouts").select("*").eq("id", layoutId).maybeSingle();
      if (error) {
    ...
    if (action === "get_order_details") {
      ...
      const { data: order_items2 } = await supabase.from("order_items").select("*").eq("order_id", o.id);
      const structuralItems = (order_items2 || []).map((m) => ({
    ...
    if (action === "search_customers") {
      ...
      const { data: usersByEmail } = await supabase.from("users").select("email").ilike("email", `%${q}%`).limit(30);
      const { data: addrMatches } = await supabase.from("user_addresses").select("user_email, fname, lname, phone, is_default").or(`fname.ilike.%${q}%,lname.ilike.%${q}%,phone.ilike.%${q}%`).limit(30);
      const { data: orderMatches } = await supabase.from("orders").select("shipping_email, shipping_fname, shipping_lname, shipping_phone, id, total, status, date").ilike("shipping_email", `%${q}%`).order("date", { ascending: false }).limit(30);
      ...
      (addrMatches || []).forEach((a) => {
        if (a.user_email) emailSet.add(a.user_email.toLowerCase().trim());
      });
      (orderMatches || []).forEach((o) => {
        if (o.shipping_email) emailSet.add(o.shipping_email.toLowerCase().trim());
      });
      if (emailSet.size === 0) {
        return new Response(JSON.stringify({ success: true, customers: [] }), { status: 200 });
      }
    ...
    const { data: order_items } = currentPageOrderIds.length > 0 ? await supabase.from("order_items").select("*").in("order_id", currentPageOrderIds) : { data: [] };
...
async function POST({ request }) {
  try {
    ...
    const adminSecretPost = "CozyAura@6969";
    if (!adminSecretPost) ;
    ...
    if (action === "add_card_template") {
      ...
      const { data: newTemplate, error } = await supabase.from("card_templates").insert([{ name, image_url: imageUrl }]).select("*").single();
      if (error) {
        return new Response(JSON.stringify({ success: false, error: error.message }), { status: 500 });
      }
    ...
    if (action === "add_sticker") {
      ...
      const { data: newSticker, error } = await supabase.from("stickers").insert([{ name, image_url: imageUrl }]).select("*").single();
      if (error) {
        return new Response(JSON.stringify({ success: false, error: error.message }), { status: 500 });
      }
    ...
    if (action === "save_product") {
      ...
      try {
        if (data.product.stock > 0) {
          await supabase.from("back_in_stock_requests").delete().eq("product_id", data.product.id).in("variant_name", ["standard", "Standard", ""]);
        }
        const restockedVariants = Object.entries(submittedFragrances).filter(([_, stockCount]) => stockCount > 0).map(([fragranceName, _]) => fragranceName);
        if (restockedVariants.length > 0) {
          ...
          await supabase.from("back_in_stock_requests").delete().eq("product_id", data.product.id).in("variant_name", variantNamesToDelete);
        }
      } catch (err) {
