...
function getShippingGroup(state) {
  if (!state) return null;
  return STATE_GROUPS[state.toLowerCase().trim()] || null;
}

function getSlabIndex(weightGrams) {
...
function getChargeForGroupWeight(group, weightGrams) {
...
function calculateShipping(items, state, pincode) {
...
async function calculateCartTotalOnServer(items, couponCode, state, pincode, deliveryMethod, isGift = false) {
  ...
  if (couponCode) {
    const { data: coupon } = await supabase.from("coupons").select("*").eq("code", couponCode.toUpperCase()).eq("status", "Active").maybeSingle();
    if (coupon) {
...
async function autoCleanExpiredLocks() {
  try {
    const { error } = await supabase.from("inventory_locks").delete().lt("expires_at", (/* @__PURE__ */ new Date()).toISOString());
    if (error) throw error;
  } catch (err) {
...
async function autoCleanInactiveVariants() {
  const now = Date.now();
  if (now - _lastAutoCleanTime < 5 * 60 * 1e3) return;
...
async function GET({ request }) {
  try {
    ...
    const { data: dbLocks } = await supabase.from("inventory_locks").select("*").gt("expires_at", (/* @__PURE__ */ new Date()).toISOString());
    const locksMap = {};
    if (dbLocks) {
...
async function POST({ request }) {
  try {
    ...
    if (body.action === "increment_views") {
      try {
        const { data } = await supabase.from("settings").select("value").eq("key", "site_views_daily").maybeSingle();
    ...
    if (body.action === "new_order") {
      ...
      for (const ri of formattedRawItems) {
        ...
        let dbStock = 0;
        if (isStandard) {
          const { data: dbProd, error: prodErr } = await supabase.from("products").select("stock").eq("id", ri.product_id).maybeSingle();
          if (prodErr || !dbProd) {
            return new Response(JSON.stringify({ success: false, error: "Insufficient stock" }), { status: 400 });
          }
          dbStock = dbProd.stock;
        } else {
          const { data: dbVar, error: varErr } = await supabase.from("product_variants").select("stock").eq("product_id", ri.product_id).eq("variant_name", ri.variant_name).maybeSingle();
          if (varErr || !dbVar) {
            return new Response(JSON.stringify({ success: false, error: "Insufficient stock" }), { status: 400 });
          }
          dbStock = dbVar.stock;
      ...
      if (!isCOD) {
        ...
        const { data: intent, error: fetchIntentErr } = await supabase.from("payment_intents").select("*").eq("razorpay_order_id", razorpayOrderId).maybeSingle();
        ...
        if (new Date(intent.expires_at) < /* @__PURE__ */ new Date()) {
      ...
      if (!isCOD && razorpayOrderId && !finalLayoutId) {
        const { data: intent } = await supabase.from("payment_intents").select("gift_card_layout_id").eq("razorpay_order_id", razorpayOrderId).maybeSingle();
    ...
    if (body.action === "add_address") {
      ...
      const { count, error: countErr } = await supabase.from("user_addresses").select("*", { count: "exact", head: true }).eq("user_email", email).neq("label", "Pickup");
    ...
    if (body.action === "get_wishlist") {
      ...
      const { data, error } = await supabase.from("wishlist").select("product_id, variant_name").eq("user_email", email);
      if (error) {
        return new Response(JSON.stringify({ success: false, error: error.message }), { status: 500 });
      }
    ...
    if (body.action === "remove_from_wishlist") {
      ...
      const { error } = await supabase.from("wishlist").delete().eq("user_email", cleanEmail).eq("product_id", product_id).eq("variant_name", cleanVariant);
      if (error) {
        return new Response(JSON.stringify({ success: false, error: error.message }), { status: 500 });
      }
      return new Response(JSON.stringify({ success: true }), { status: 200 });
    ...
    if (body.action === "track_order") {
      ...
      const { data: matchedOrders, error: orderFetchErr } = await supabase.from("orders").select("*").or(`id.eq.${sanitizedQuery},shipping_email.eq.${sanitizedVal},shipping_phone.ilike.%${sanitizedVal}%`).order("date", { ascending: false }).limit(50);
      if (orderFetchErr) {
      ...
      const { data: orderItems } = await supabase.from("order_items").select("*").in("order_id", matchedIds);
    ...
    if (body.action === "submit_feedback") {
      ...
      const { data: orderRow } = await supabase.from("orders").select("shipping_email, shipping_fname, shipping_lname").eq("id", orderId).maybeSingle();
