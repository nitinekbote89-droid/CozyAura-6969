...
export async function sendOrderShipped({ email, name, orderId, trackingNumber, courier, trackingLink, deliveryMethod, siteOrigin, items, subtotal, discount, shipping, total, address }) {
  const isPickup = deliveryMethod === 'Pickup';
  const formattedOrderId = String(orderId).startsWith('#') ? orderId : `#${orderId}`;
