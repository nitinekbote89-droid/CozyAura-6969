-- SQL Patch to create gift_card_layouts table and integrate it into orders
-- Run this in your Supabase SQL Editor: https://supabase.com/dashboard/project/_/sql/new

-- 1. Create gift_card_layouts table to store coordinates and templates
CREATE TABLE IF NOT EXISTS gift_card_layouts (
    id TEXT PRIMARY KEY,
...
-- 2. Add columns to orders and payment_intents to link layouts
ALTER TABLE orders ADD COLUMN IF NOT EXISTS gift_card_layout_id TEXT REFERENCES gift_card_layouts(id) ON DELETE SET NULL;
ALTER TABLE payment_intents ADD COLUMN IF NOT EXISTS gift_card_layout_id TEXT;

-- 3. Drop the old function before redefining it with the new signature
DROP FUNCTION IF EXISTS place_order_securely(TEXT, TEXT, TEXT, TEXT, TEXT, BIGINT, TEXT, TEXT, TEXT, TEXT, TEXT, NUMERIC, TEXT, JSONB, TEXT, NUMERIC, NUMERIC, TEXT);

-- 4. Redefine place_order_securely with the new p_gift_card_layout_id parameter
CREATE OR REPLACE FUNCTION place_order_securely(
    ...
    p_gift_card_layout_id TEXT DEFAULT NULL
) RETURNS TEXT AS $$
DECLARE
...
BEGIN
    -- 1. Idempotency check: verify if payment was already processed
    BEGIN
    ...
    -- 2. Fetch expected total from payment_intents if razorpay_order_id is supplied
    ...
    -- 6. Insert order (includes denormalized shipping details & gift card ID)
    INSERT INTO orders (
